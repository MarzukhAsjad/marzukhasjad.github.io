          ...
          <article className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <ReactMarkdown
              ...
              components={{
                ...
                blockquote: ({ children }) => (
                  <blockquote className="border-l-4 border-yellow-500 bg-yellow-900/20 pl-4 pr-4 py-3 rounded-r-lg italic text-yellow-200 mb-4 font-mono min-h-[3rem] flex items-center [&>p]:mb-0">
                    {children}
                  </blockquote>
                ),
              }}
...
But microservices are not always overkill, sometimes they are necessary. For example, you just onboarded a company as a project manager, and suddently you find out your company have some legacy REST APIs built in Java or PHP. What's the first thing you do? Migrate that PHP API to Python FastAPI? Or perhaps, rewrite that Java API to Node.js Express? **NAW!**

> If it ain't broke, don't fix it.

But the staff which created this legacy API is no longer with the company or is retired or is busy with other things. But your new task involves integrating this legacy API with a new functionality. What do you do here? The best approach is to leave the legacy API as it is, and build a new microservice for that functionality that can communicate with it. This is where webhooks come in handy. I hate to break it to you, but you are still going to have to learn that PHP or Java framework to some extent to expand on it to add webhook functionality. But hey, at least you are not creating the new microservice in that legacy framework.
